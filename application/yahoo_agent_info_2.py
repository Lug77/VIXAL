...
name_program = 'yahoo_agent_info_2'
comment_prog = 'получение сектора, индустрии, рекоммендаций и таргета с сайта finance.yahoo.com'
...
def crud_config(path_):
    ...
    # программа используется только для базы usd, поэтому сразу выбираем нужный путь из config-файла
    ...
    if r is not False:
        ...
        if market_cap > 0:
            if market_cap > 100000000000:
                market_cap_str = 'Big'
            elif (market_cap < 100000000000) and (market_cap > 10000000000):
                market_cap_str = 'Large'
            elif (market_cap < 10000000000) and (market_cap > 3000000000):
                market_cap_str = 'Middle'
            elif (market_cap < 3000000000) and (market_cap > 1000000000):
                market_cap_str = 'Small'
            else:
                market_cap_str = 'Micro'
...
####################################################################################################
logging.basicConfig(filename='yahoo_agent_info_2.log',
...
if sql_obj.check_db(db_name) is False:
    ...
    if sql_obj.check_table_is_exists(cursorObj, name_main_table):
        ...
        for row in rows:
            # перебор начинаем с указанного индекса задается в settings_expert_system.ini
            # (сраниваем с primary key)
